...
class TrelloRepository:
    ...
    def refresh_lists_cache(self):
        ...
        self.lists_cache = dict(zip(
            [api_list['id'] for api_list in api_lists],
            [TrelloList(api_list['id'], api_list['name']) for api_list in api_lists]
            ))
    ...
    def get_board_cards(self) -> list[TrelloCard]:
        response = requests.get(f'{self.board_url}/cards', params=self.base_query_params)
        return [TrelloCard(api_card['id'], api_card['name'], api_card['idList']) for api_card in response.json()]
